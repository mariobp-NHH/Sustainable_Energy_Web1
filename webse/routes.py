...
from webse.forms import RegistrationForm, LoginForm, UpdateAccountForm, PostForm, PostModuls
from webse.models import User, Post, Moduls
...
# App web. Module2.
@app.route('/app_web/module2', methods=['GET', 'POST'])
@login_required
def app_web_module2():
    form = PostModuls()
    if form.validate_on_submit():
        moduls = Moduls(module1_q1_string=form.module1_q1_string.data, user_id=1)
        db.session.add(moduls)
        db.session.commit()
        flash('Your answer has been submitted!', 'success')
        return redirect(url_for('account'))
    return render_template('app_web_module2.html', title='App Web Module2',
                           form=form)